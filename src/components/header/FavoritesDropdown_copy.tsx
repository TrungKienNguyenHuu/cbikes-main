...
interface FavoritesDropdownCopyProps {
  favorites: Bike[];
  isOpen: boolean;
  onClose: () => void;
}

export const FavoritesDropdownCopy = memo(
  ({ favorites, isOpen, onClose }: FavoritesDropdownCopyProps) => {
    const navigate = useNavigate();

    if (!isOpen) return null;

    const handleItemClick = (bikeId: string) => {
      // Route to test product details page
      navigate(`/test/product/${bikeId}`);
      onClose();
    };

    return (
      <Dropdown>
        <DropdownTitle>Favorite Products (Test Backend)</DropdownTitle>
        {favorites.length === 0 ? (
