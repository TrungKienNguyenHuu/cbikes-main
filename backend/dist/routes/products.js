...
const aggregatePriceHistory = (listings) => {
...
router.get("/metadata", async (req, res) => {
    try {
      ...
      ORDER BY b.name
    `;
        const catResult = await database_1.default.query(catQuery);
      ...
      ORDER BY plat.name
    `;
        const sellResult = await database_1.default.query(sellQuery);
...
router.get("/", async (req, res) => {
    try {
        const { page = "1", limit = "12", search = "", category = "all", seller = "all", minPrice, maxPrice, need = "all", tier = "all", sort = "updated-asc" } = req.query;
    ...
    `;
        ...
        const products = result.rows.map((row) => {
            ...
            if (listings.length > 0) {
                lowestPrice = listings.reduce((lowest, current) => current.price < lowest.price ? current : lowest).price;
            }
