...
const seedData = async () => {
  try {
    ...
    const platformIdMap: { [key: string]: string } = {};
    for (const platform of platforms) {
      const result = await pool.query(
        `INSERT INTO platforms (name, slug, is_marketplace)
         VALUES ($1, $2, $3)
         RETURNING platform_id`,
        [platform.name, platform.slug, platform.isMarketplace]
      );
    ...
    for (const product of productsData) {
      ...
      await pool.query(
        `INSERT INTO products (product_id, brand_id, name, slug, image_url, created_at)
         VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)`,
        [productId, brandId, `${product.brandSlug} ${product.name}`, slug, product.imageUrl]
      );
    }
